import { register } from './PriceError';

const Messages = {
	CLIENT_INVALID_OPTION: (prop: any, must: any) => `The ${prop} option must be ${must}`,
	...
	WS_NOT_OPEN: (data: any) => `Websocket not open to send ${data}`,
	...
	SHARDING_ALREADY_SPAWNED: (count: number) => `Already spawned ${count} shards.`,
	SHARDING_PROCESS_EXISTS: (id: number) => `Shard ${id} already has an active process.`,
	SHARDING_READY_TIMEOUT: (id: number) => `Shard ${id}'s Client took too long to become ready.`,
	SHARDING_READY_DISCONNECTED: (id: number) => `Shard ${id}'s Client disconnected before becoming ready.`,
	SHARDING_READY_DIED: (id: number) => `Shard ${id}'s process exited before its Client became ready.`,
	...
	VOICE_CONNECTION_ATTEMPTS_EXCEEDED: (attempts: number) => `Too many connection attempts (${attempts}).`,
	...
	VOICE_STATE_INVALID_TYPE: (name: string) => `${name} must be a boolean.`,
	...
	MISSING_MANAGE_EMOJIS_PERMISSION: (guild: string) =>
